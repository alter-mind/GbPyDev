...
На вход программе подается список arr, где arr[i] (1 ≤ arr[i] ≤ 1000) - урожайность i-го куста черники. Размер списка не
превышает 1000 элементов.
...
# arr = [randrange(1, 20) for el in range(randrange(5, 10))]
# print(arr)
arr_f = arr + arr[:2]
print(max([arr_f[i] + arr_f[i + 1] + arr_f[i + 2] for i in range(len(arr))]))

'''
Обратите внимание, что грядка круглая - т.е. нам нужно как бы "приклеить" в конец два первых элемента, чтобы вычислить
сумму для каждых трёх соседних кустов, на случай если максимальная сумма будет захватывать обе границы массива
'''